...
class Category extends \MageWorx\SeoExtended\Model\MetaUpdater
{
    ...
    public function update($onlyFilterReplace = false)
    {
        ...
        if ($onlyFilterReplace) {
            if ($this->updateTitleBySeoFilter()) {
                $stopLnFiltersInTitle = true;
            }
        } else {
            $this->updateTitle($stopLnFiltersInTitle);
        }
        ...
        if ($onlyFilterReplace) {
            if ($this->updateMetaDescriptionBySeoFilter()) {
                $stopLnFiltersInMetaDescription = true;
            }
        } else {
            $this->updateMetaDescription($stopLnFiltersInMetaDescription);
        }
        //Meta Keywords:
    ...
    protected function getStringByFilters()
    {
        if ($this->filtersString === null) {
             ...
             */
            $layeredFiltersGetterModel = $this->filtersConvertorFactory->create($this->request->getActionName());
