...
class Router extends \Magento\UrlRewrite\Controller\Router
{
    ...
    public function match(\Magento\Framework\App\RequestInterface $request)
    {
        ...
        // --- Customize to change magebrand to brand in url --//
        if(isset($urlData['params']['mw_ln']['magebrand'])) {
            $urlData['params']['mw_ln']['brand'] = $urlData['params']['mw_ln']['magebrand'];
            unset($urlData['params']['mw_ln']['magebrand']);
        }
        // --------//
        ...
        if ($rewrite === null) {
            ...
            $request->setAlias(\Magento\Framework\UrlInterface::REWRITE_REQUEST_PATH_ALIAS, $urlData['path']);
            $request->setPathInfo('/' . $urlData['path']);
            ...
            if ($this->helperData->getIsInvertRedirectEnable()
                ...
                $defaultUrl = $this->url->getUrl('', ['_direct' =>$urlData['path'], '_query' => $params]);
