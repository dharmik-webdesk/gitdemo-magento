...
//        $this->checkProductBasedAvailability();
		...
		$this->paymentForm = $this->objectManager->get('Splitit\Paymentmethod\Model\PaymentForm');
		$this->checkoutSession = $this->objectManager->get('\Magento\Checkout\Model\Session');
		$this->countryFactory = $this->objectManager->get('\Magento\Directory\Model\CountryFactory');
	}
	...
	public function authorize(\Magento\Payment\Model\InfoInterface $payment, $amount) {
		...
		*/
		...
		try {
			...
			/**Check API order total and Magento order total*/

			$apiPlanDetails = $this->paymentForm->getInstallmentPlanDetails($this->_apiModel);
			$this->_logger->error('=============================<><>=============');
			$this->_logger->error(print_r($apiPlanDetails, true));
			$orderGrandTotal = $this->grandTotal;
			//	$this->_logger->error(print_r($orderGrandTotal, true));
			//$orderGrandTotal = round($this->checkoutSession->getQuote()->getGrandTotal(), 2);
			if ($apiPlanDetails['grandTotal'] !== $orderGrandTotal || $apiPlanDetails['numberOfInstallments'] !== $this->checkoutSession->getSelectedIns()) {
				//$this->_logger->error('ddd - ');
				//$this->_logger->error(print_r($this->checkoutSession->getSelectedIns(), true));
				//$this->_logger->error('ddd - kkk -');
				//$this->_logger->error(print_r($this->checkoutSession->getQuote()->getCustomerEmail(), true));
				$responseInit = $this->_apiModel->installmentPlanInit($this->checkoutSession->getSelectedIns(), $this->checkoutSession->getQuote()->getCustomerEmail());
				//$this->_logger->error(print_r($responseInit, true));
			}
			...
			$payment->setAdditionalInformation(
				[\Magento\Sales\Model\Order\Payment\Transaction::RAW_DETAILS => (array) $result, 'method_title' => $payment->getMethod()]
			);
	...
	protected function createInstallmentPlan($api, $payment, $amount) {
		...
		if ($this->isOneInstallment()) {
			...
			if (isset($billingAddress->getStreet()[0])) {
				$billingStreet1 = $billingAddress->getStreet()[0];
			}
			if (isset($billingAddress->getStreet()[1])) {
				$billingStreet2 = $billingAddress->getStreet()[1];
			}
			...
			$params = [
				...
				"BillingAddress" => [
					...
					"Country" => $this->countryFactory->create()->loadByCode($billingAddress->getCountry())->getName('en_US'),
