...
class Api extends \Magento\Payment\Model\Method\AbstractMethod {
	...
	public function installmentPlanInit($selectedInstallment, $guestEmail) {
		...
		$params = $this->createDataForInstallmentPlanFirstInitCall($selectedInstallment);
	...
	public function createDataForInstallmentPlanFirstInitCall($selectedInstallment) {
...
//        print_r($params);
	...
	public function checkForBillingFieldsEmpty() {
		...
		$this->logger->error(print_r($this->billingAddress->getData(), true));
		$this->logger->error(print_r($this->billingAddress->getStreet(), true));
		$response = ["errorMsg" => "", "successMsg" => "", "status" => false];
