...
//                        var textToShowPdp = window.currencySymbolToShow + installmentsCalculatedPdp + ' x ' + window.installmentsCount + ' ' + window.installmetPriceTextToShow;
                        var textToShowPdp = 'or '+window.installmentsCount+' payments of '+ window.currencySymbolToShow + installmentsCalculatedPdp ;//+ ' ' + window.installmetPriceTextToShow;
		...
		success: function(result){
			...
			if(result.isActive){
				...
				if(jQuery("table.table-totals").length && displayInstallmentPriceOnPage.indexOf("checkout") >= 0){
					...
					if(parseFloat(productprice)){
						installments = (productprice/result.numOfInstallmentForDisplay).toFixed(2);
						installmentNewSpan = '<span class="cart-installment-onepage">'+currencySymbol+installments+' x '+result.numOfInstallmentForDisplay+' '+result.installmetPriceText+'</span>';
						jQuery('table.table-totals').find('.cart-installment-onepage').closest('tr').remove();
						jQuery('table.table-totals tr:last').after('<tr><td colspan="2">'+installmentNewSpan+'</td></tr>');
					}
