 ...
 */
namespace Wds\Coreoverride\Controller\Adminhtml\Category;
    ...
    public function execute()
    {
        ...
        if ($categoryPostData) {
            ...
            try {
				...
				// KM custom code
				 if(isset($categoryPostData['image_thumb'])){
					if(count($categoryPostData['image_thumb'])>0){
						$thumbimage = $categoryPostData['image_thumb'][0]['name'];
						$imageUploader = $this->_objectManager->get(\Magento\Catalog\CategoryImageUpload::class);
						if ($thumbimage !== NULL && $thumbimage!="") {
							try {
								$imageUploader->moveFileFromTmp($thumbimage);
							} catch (\Exception $e) {
